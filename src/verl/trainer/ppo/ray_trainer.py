...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    elif adv_estimator == AdvantageEstimator.GBPO:
        advantages, returns, target_adv = core_algos.compute_gbpo_outcome_advantage_and_target(
            token_level_rewards=data.batch["token_level_rewards"],
            response_mask=data.batch["response_mask"],
            index=data.non_tensor_batch["uid"],
            config=config,
        ...
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
        data.batch["gbpo_target_adv"] = target_adv
    else:
...
class RayPPOTrainer:
    ...
    def _validate_config(self):
        ...
        if config.actor_rollout_ref.rollout.multi_turn.enable:
            ...
            assert config.algorithm.adv_estimator in [AdvantageEstimator.GRPO, AdvantageEstimator.GBPO], (
                "only GRPO/GBPO are supported for multi-turn with tool"
            )
